...
EvaluationMetrics CalculateMetrics(const std::vector<int>& groundTruths, 
    ...
    for (size_t c = 0; c < numClasses; ++c) {
        ...
        for (size_t i = 0; i < numClasses; ++i) {
            classSupport += confusionMatrix[c][i]; // Class ground truth count
            if (i != c) {
...
int main()
{
    try
    {
        ...
        const std::string testImagesDir = "test_images"; // cnn_gpu/test_images µð·ºÅÍ¸® ÁöÁ¤
        ...
        // Single Image Path for baseline fallback
        const std::string singleImagePath = "results/test_wafer.png";

        // =========================================================
        // 2. ONNX Runtime Environment & CUDA Initialization
        // =========================================================
        std::cout << "[1] Creating ONNX Runtime environment..." << std::endl;
        Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "WaferInference");
        Ort::SessionOptions sessionOptions;
        sessionOptions.SetIntraOpNumThreads(1);
        sessionOptions.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_ALL);

        std::cout << "[2] Configuring CUDA Execution Provider..." << std::endl;
        try {
            ...
            std::cout << "[SUCCESS] CUDA Execution Provider enabled." << std::endl;
        }
        catch (const Ort::Exception& e) {
            std::cerr << "[WARNING] Failed to enable CUDA EP. Falling back to CPU. Info: " << e.what() << std::endl;
        }

        std::cout << "[3] Loading FP32 ONNX Model..." << std::endl;
        Ort::Session session(env, modelPath.c_str(), sessionOptions);
        std::cout << "[SUCCESS] FP32 ONNX model loaded!" << std::endl;
        ...
        // =========================================================
        // 3. Single Sample Inference & 100 Warm-up Benchmark
        // =========================================================
        std::cout << "\n[4] Running Single Image Baseline & Warm-up (100 runs)..." << std::endl;
        
        cv::Mat sampleImg = cv::imread(singleImagePath, cv::IMREAD_GRAYSCALE);
        if (sampleImg.empty()) {
            std::cerr << "[WARNING] Could not load sample image (" << singleImagePath << "). Creating dummy tensor for warm-up." << std::endl;
            sampleImg = cv::Mat::zeros(24, 24, CV_8UC1);
        }

        cv::Mat sampleFloat, sampleResize;
        sampleImg.convertTo(sampleFloat, CV_32F);
        double minV, maxV;
        cv::minMaxLoc(sampleFloat, &minV, &maxV);
        if (maxV > 0.0) sampleFloat /= static_cast<float>(maxV);
        cv::resize(sampleFloat, sampleResize, cv::Size(24, 24), 0, 0, cv::INTER_LINEAR);

        std::vector<float> sampleInputData(24 * 24);
        std::copy(sampleResize.ptr<float>(), sampleResize.ptr<float>() + 24 * 24, sampleInputData.begin());

        Ort::Value sampleTensor = Ort::Value::CreateTensor<float>(
            memoryInfo, sampleInputData.data(), sampleInputData.size(), inputShape.data(), inputShape.size()
        );

        // Warm-up runs
        for (int i = 0; i < 100; ++i) {
            session.Run(Ort::RunOptions{ nullptr }, inputNames, &sampleTensor, 1, outputNames, 1);
        }
        std::cout << "[SUCCESS] 100 Warm-up runs completed." << std::endl;

        // =========================================================
        // 4. Test Set Evaluation (3,828 Images Pipeline Benchmark)
        // =========================================================
        ...
        if (fs::exists(testImagesDir) && fs::is_directory(testImagesDir)) {
            for (const auto& entry : fs::directory_iterator(testImagesDir)) {
                if (entry.is_regular_file() && entry.path().extension() == ".png") {
                    ...
                    // Filename format parsing: e.g., "00001_label2.png" or "104496_label2_Edge-Loc.png"
        ...
        if (imagePaths.empty()) {
            std::cout << "[INFO] 'test_images' directory empty or not found." << std::endl;
            std::cout << "[INFO] Executing fallback 3828-run pipeline benchmark on single image baseline." << std::endl;
            imagePaths.resize(3828, singleImagePath);
            groundTruths.resize(3828, 2); // Dummy label
        }
        ...
        for (size_t idx = 0; idx < imagePaths.size(); ++idx) {
            ...
            size_t outCount = outputTensors[0].GetTensorTypeAndShapeInfo().GetElementCount();
            int predClass = static_cast<int>(std::distance(outData, std::max_element(outData, outData + outCount)));
        ...
        // =========================================================
        // 5. Latency & Throughput Benchmark Analysis
        // =========================================================
        ...
        std::cout << "\n========================================\n";
        std::cout << "  P2 Research Benchmark Metrics (GPU)\n";
        std::cout << "========================================\n";
        ...
        std::cout << "\n[SUCCESS] Pipeline Benchmark Complete." << std::endl;
        return 0;
    }
    catch (const Ort::Exception& e) {
